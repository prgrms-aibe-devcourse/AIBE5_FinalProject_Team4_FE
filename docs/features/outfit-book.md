...
| `OUTFITBOOK-002` | 코디북 조회 | 저장된 코디 목록을 조회하고 목록 화면에 표시합니다. 전체/즐겨찾기 탭 전환과 코디별 즐겨찾기 토글을 지원합니다. |
| `OUTFITBOOK-003` | 코디북 상세 | 저장된 코디의 스타일 상세 정보와 구성 옷을 확인합니다. |
| `OUTFIT-001`~`OUTFIT-006` | 코디 저장/조회/수정/삭제 | 추천받거나 직접 구성한 코디를 저장, 조회, 수정, 삭제합니다. 코디 수정 시 즐겨찾기 상태도 함께 유지됩니다. |
...
- 즐겨찾기탭(OUTFITBOOK-TAB-FAVORITE)은 전체 코디 중 `favorite: true`인 항목만 필터링하여 표시합니다.
