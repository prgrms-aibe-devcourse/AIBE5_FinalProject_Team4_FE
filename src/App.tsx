...
// Í∏∞Ï°¥ ?ÅÏàò data ( TRIGGER_PRODUCTS ???¨Ïö©???òÏ??äÏïÑ ?∞ÏÑ† Ï£ºÏÑùÏ≤òÎ¶¨??)
...
export default function App() {
  ...
  // ÎπÑÎ°úÍ∑∏Ïù∏ ?ÅÌÉúÎ©?Î™®Îã¨???¥Í≥† falseÎ•?Î∞òÌôò, Î°úÍ∑∏???ÅÌÉúÎ©?trueÎ•?Î∞òÌôò
  ...
  useEffect(() => {
    api.get('/api/v1/users/profile')
        .then((res) => {
          ...
          if(onboarded){
            // ?®Î≥¥???ÑÎ£å ?†Ï?: nickname + onboarded: true
            setProfile(prev => ({ ...prev, nickname, onboarded: true }));
          } else if(nickname){
            // ?®Î≥¥??ÎØ∏ÏôÑÎ£åÏ?Îß?OAuth ?âÎÑ§??Ï°¥Ïû¨: Í∏∞Î≥∏Í∞íÏúºÎ°úÎßå ?úÏö©
            ...
            try {
              ...
              if (authUserId != null) {
                try {
                  ...
                  alert("ÎßàÏ????ïÎ≥¥ ?òÏã† ?ôÏùò ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§. ÎßàÏù¥?òÏù¥ÏßÄ?êÏÑú ?§Ïãú Î≥ÄÍ≤ΩÌï† ???àÏäµ?àÎã§.");
                }
              }
              ...
              alert('?ÑÎ°ú???Ä?•Ïóê ?§Ìå®?àÏñ¥?? ?§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.');
            }
          }} />
      )}
          ...
          <div className="flex-1 flex flex-col justify-between overflow-hidden relative bg-[#F8FAFC]">
            ...
            <header className="fixed top-0 left-0 right-0 bg-white border-b border-slate-100 px-5 py-4.5 flex items-center justify-between z-30">
              ...
              >
                ...
                <div className="brand-wordmark" aria-label="?∑Ïû•?úÍ∞ê">
                  <span>??/span>
                  <span>??/span>
                  <span>??/span>
                  <span>Í∞?/span>
              ...
              <div className="flex items-center space-x-4">
                ...
                <button
                  onClick={() => {
                    alert("?îî ?àÎ°ú???åÎ¶º???ÜÏäµ?àÎã§. Í∞êÍ∞Å?¥Ï? ?®ÏÖò Îß§Ïπ≠ Ï§ÄÎπÑÍ? ?ÑÎ£å?òÏóà?µÎãà??");
                  }}
              ...
              {currentTab === "closet" &&
                  ...
                  authUserId == null && (
                      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-center text-sm text-amber-900">
                        Î°úÍ∑∏???ïÎ≥¥Î•??ïÏù∏?????ÜÏäµ?àÎã§. ?§Ïãú Î°úÍ∑∏?∏Ìï¥ Ï£ºÏÑ∏??
                      </div>
                  )}
              ...
              {currentTab === "feed" && (
                <div className="space-y-5 animate-fade-in text-left">
                  <div className="space-y-1">
                    <h3 className="text-base font-bold text-[#1E3A8A]">Closet Toy ?§Ì????§ÏãúÍ∞??ºÎìú</h3>
                    <p className="text-xs text-slate-400">?§Î•∏ ?¥Î????§Îßà???†Ï??§Ïùò ?∏Ï≤¥ Ï°∞Ìôî ?§Ì??ºÎßÅ ?§ÎÉÖ?∑ÏùÑ Íµ¨Í≤Ω?òÏÑ∏??</p>
                  </div>
                    ...
                    <div className="p-4 space-y-2">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center space-x-1.5">
                          <span className="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center text-[10px]">?§ñ</span>
                          <span className="text-xs font-bold text-slate-700">@tech_minimalist_kim</span>
                        </div>
                        <span className="bg-[#BBF7D0] text-[#1E3A8A] text-[10px] font-bold px-2 py-0.5 rounded-full">94% ÎßåÏ°±</span>
                      </div>
                      <p className="text-xs text-slate-500">"Í≥†Ï†ïÎ∞Ä ???åÌÅ¨?àÏª¨ ?ÑÎÖ∏?ΩÏùÑ ÏΩîÎîî Ï∂îÏ≤úÎ∞õÏïÑ Ï∞©Ïö©?¥Î≥¥???¥Íπ® ?ºÏù∏Î∂Ä???êÎ•¥??drape ?èÏù¥ ?ïÎßê ÎßàÏùå???§Ïñ¥??"</p>

                      <div className="flex items-center space-x-1.5 text-[10px] text-[#1E3A8A] font-semibold bg-slate-50 p-2 rounded-lg">
                        <Activity className="w-3.5 h-3.5" />
                        <span>?òÎ•ò Ï°∞Ìôî ?àÏù¥?¥ÎßÅ ÏßÄ???∞Ïàò</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-xs">
                    ...
                    <div className="p-4 space-y-2">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center space-x-1.5">
                          <span className="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center text-[10px]">?ëü</span>
                          <span className="text-xs font-bold text-slate-700">@ame_gorp_lover</span>
                        </div>
                        <span className="bg-[#BBF7D0] text-[#1E3A8A] text-[10px] font-bold px-2 py-0.5 rounded-full">91% ÎßåÏ°±</span>
                      </div>
                      <p className="text-xs text-slate-500">"?àÎ≤ÖÏßÄÍ∞Ä ÍµµÏ? Ï∂ïÌïò??Í≥®Îã® Íµ¨Ï°∞?∏Îç∞ ?Ä?¥Îìú ?ÄÎπÑÏ? ?¨Ï∏†Í∞Ä Í≥†Í???Î≥µÎ? ?ïÎ∞ï?ÜÏù¥ Í∞ÄÎ≥çÍ≤å ?ÑÎûòÎ°??êÎ•¥?§Ïöî."</p>

                      <div className="flex items-center space-x-1.5 text-[10px] text-[#1E3A8A] font-semibold bg-slate-50 p-2 rounded-lg">
                        <Activity className="w-3.5 h-3.5" />
                        <span>Í≥®Î∞ò ?Ä?¥Í≥® Í∞Ä?ôÏó≠ 98% ?ïÎ≥¥</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {currentTab === "profile" && (
                <div className="space-y-5 animate-fade-in text-left">
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-100 text-center space-y-3 relative overflow-hidden">
                    ...
                    <div className="flex justify-center space-x-3 text-xs pt-1">
                      <span className="px-2.5 py-1 rounded-md bg-slate-50 border border-slate-100 text-slate-600 font-bold">
                        ?±Î≥Ñ: {profile.gender === "Male" ? "?®ÏÑ±" : "?¨ÏÑ±"}
                      </span>
                      <span className="px-2.5 py-1 rounded-md bg-slate-50 border border-slate-100 text-slate-600 font-bold">
                        ?§Ì??? {profile.styles.join(", ")}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-[#BBF7D0]/10 border border-[#BBF7D0]/20 space-y-1.5">
                    ...
                    <button
                      onClick={() => {
                        if (confirm("Ï¥àÍ∏∞ ?®Î≥¥?©ÏúºÎ°??òÎèå?ÑÍ??úÍ≤†?µÎãàÍπ?")) {
                         resetAuthState();
                        }
                      }}
            ...
            <div className="fixed bottom-36 right-5 z-40">
              <button
                ...
                title="?òÏù¥ÏßÄ Îß??ÑÎ°ú"
                aria-label="?òÏù¥ÏßÄ Îß??ÑÎ°ú ?¥Îèô"
                type="button"
              >
            ...
            <nav id="bottom-dock" className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-100 flex justify-around items-center px-4 shadow-xl z-30">
              ...
              >
                ...
                <span className="text-[9px] font-extrabold mt-1">Ï∂îÏ≤ú</span>
              </button>
              ...
              >
                ...
                <span className="text-[9px] font-extrabold mt-1">?∑Ïû•</span>
              </button>
              ...
              >
                ...
                <span className="text-[9px] font-extrabold mt-1">ÏΩîÎîîÎ∂?/span>
              </button>
              ...
              >
                ...
                <span className="text-[9px] font-extrabold mt-1">?ºÎìú</span>
              </button>
            </nav>
            ...
            <div className="bg-white rounded-t-[32px] h-[550px] flex flex-col overflow-hidden shadow-2xl relative">
              ...
              <div className="bg-[#1E3A8A] text-white p-5 flex items-center justify-between pb-4">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold">Í∞êÍ∞Å???Ä??1:1 ?§Ì???Ï°∞Ìôî ??/h3>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-50">
                {chatMessages.map((m, idx) => (
                  ...
                  >
                    {m.sender === "gamyagi" && (
                      <span className="text-lg p-1 bg-white border rounded-lg">?§ñ</span>
                    )}
                ...
                {chatSending && (
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-xs p-3 rounded-2xl rounded-tl-none bg-white text-[#1E3A8A] font-semibold flex items-center space-x-2 border border-slate-100">
                      ...
                      <span>Í∞êÍ∞Å?¥Í? ?Ä?¥Ìïë?òÍ≥† ?àÏñ¥??..</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="p-3 bg-white border-t border-slate-100 flex items-center space-x-2">
                <input
                  type="text"
                  placeholder="?? ???¥Íπ®Í≥®Í≤© 70???ÑÎÖ∏???èÏù¥ ?¥Ïö∏Î¶¥Ï? Î¨ºÏñ¥Î≥¥Í∏∞"
                  value={pendingMsg}
