...
/** POST /api/v1/feed/posts/{postId}/saves — 피드 코디를 내 코디북에 저장/취소 (좋아요와 별개) */
export async function toggleFeedOutfitSave(postId: number): Promise<FeedInteraction> {
  return unwrap(
    api.post<BeApiResponse<FeedInteraction>>(`${FEED_BASE}/posts/${postId}/saves`),
  )
}

/** GET /api/v1/feed/posts/{postId}/comments — FEED-005 */
...
/** POST /api/v1/feed/posts/{postId}/comments — FEED-005/006 */
...
/** POST /api/v1/feed/users/{followeeId}/follows — FEED-007 */
